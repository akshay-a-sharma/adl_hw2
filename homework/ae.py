...
class PatchAutoEncoder(torch.nn.Module, PatchAutoEncoderBase):
    ...
    class PatchEncoder(torch.nn.Module):
        ...
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            super().__init__()
            # Convert from latent_dim to bottleneck
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            features = hwc_to_chw(patches)
            features = self.non_linear(features)
            # Convert to HWC to patchify
            return chw_to_hwc(features) 

    class PatchDecoder(torch.nn.Module):
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            super().__init__()
            # Convert from bottleneck to latent_dim
            self.non_linear = torch.nn.Sequential(
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            features = hwc_to_chw(x)
            features = self.non_linear(features)
            # Convert back to HWC and unpatchify
            features = chw_to_hwc(features)
            return self.unpatchify(features)
