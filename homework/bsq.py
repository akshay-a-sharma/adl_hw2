...
class BSQ(torch.nn.Module):
    def __init__(self, codebook_bits: int, embedding_dim: int):
        ...
        self.codebook_projection = torch.nn.Linear(embedding_dim, codebook_bits)
        self.embedding_projection = torch.nn.Linear(codebook_bits, embedding_dim)
...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def decode_index(self, x: torch.Tensor) -> torch.Tensor:
        binary = self.bsq._index_to_code(x)
        return super().decode(self.bsq.decode(binary))
